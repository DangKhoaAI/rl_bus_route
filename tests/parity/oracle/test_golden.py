...
from bus_sim.parity.fixtures import load_fixture, replay_explicit, replay_fixture
from bus_sim.parity.scenarios import CATALOG
from bus_sim.parity.snapshot import compare_records, first_divergence
from tests.support.paths import GOLDEN


@pytest.mark.parametrize("name", sorted(CATALOG))
def test_fixture_replays_exactly(name):
    input_payload, expected, _events = load_fixture(GOLDEN / name)
...
def test_corrupted_fixture_reports_first_divergence():
    _input_payload, expected, _events = load_fixture(GOLDEN / "normal_m3")
...
def test_events_are_action_driven_and_complete():
    ...
    for name in sorted(CATALOG):
        input_payload, _, events = load_fixture(GOLDEN / name)
