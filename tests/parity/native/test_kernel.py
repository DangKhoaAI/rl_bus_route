...
These tests are skipped when the `bus_sim_native` extension has not been built
...
from tests.support.contract import KERNEL_COMPARE_KEYS
from tests.support.paths import GOLDEN
from tests.support.rust_bridge import kernel_flat, make_kernel, run_actions
...
def test_kernel_matches_oracle_events(name):
    payload, _, events = load_fixture(GOLDEN / name)
...
def test_kernel_rejects_invalid_action_before_mutation():
    payload, _expected, _ = load_fixture(GOLDEN / "normal_m3")
...
def test_kernel_terminal_settlement_is_charged_once():
    ...
    payload, _, _ = load_fixture(GOLDEN / "backlog_m3")
    flat = kernel_flat(payload["spec"], payload["actions"])
...
def test_kernel_consumes_config_ticks_per_interval():
    payload, _, _ = load_fixture(GOLDEN / "zero_m3")
    flat = kernel_flat(payload["spec"], payload["actions"])
