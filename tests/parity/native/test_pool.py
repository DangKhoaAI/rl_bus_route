...
from tests.support.compare import assert_frames, first_action_divergence
...
def test_runtime_defaults_keep_scalar_and_training_hyperparameters():
    ...
    assert hasattr(bus_sim_native, "BatchKernel")
...
def test_batch_one_via_pool_matches_scalar(reference_bundle):
    ...
    try:
        ...
        pool.close()
    assert_frames(scalar, batched)
    divergence = first_action_divergence(scalar_traces, batched_traces)
...
def test_pool_invalidates_on_order_flags_config_forecast_and_contract(reference_bundle):
    ...
    try:
        ...
        config_changed = replace(run, runtime=replace(run.runtime, validate_observation=False))
...
def test_pool_does_not_reuse_metrics_across_checkpoint_loads(reference_bundle):
    ...
    try:
        ...
        assert_frames(second, scalar_after)
    finally:
...
def test_batch_forecast_matches_scalar_or_errors_clearly(reference_bundle):
    ...
    batched_run = replace(forecast_run, runtime=replace(forecast_run.runtime, eval_batch_size=4))
