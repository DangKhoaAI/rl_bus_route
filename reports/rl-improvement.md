...
checkpoint were not reused as L0/L3 quality evidence. The L3 held-out splits and
T9 matrix were opened only after `l3_protocol.json` froze the analysis; the
fixed-wall-clock budget is registered but reduces to the observed fixed-transition
wall clock because no adopted algorithm change alters per-step compute. No global
defaults were changed and no candidate was adopted or combined.

Limitation: the L3 figures were regenerated by `scripts/l3_analysis.py` and their
axes, units and underlying ranges were checked against `l3_paired_days.csv` /
`l3_held_out_summary.csv`; a human visual pass over the rendered PNGs should
still accompany any downstream reuse.
