...
This directory contains the frozen L0/L1 protocol and curated evidence. Raw
...
- [`experiments.csv`](experiments.csv) is the pre-registered experiment ledger;
  every L1 candidate must have a complete card before its first run.
- [`baseline.md`](baseline.md) contains the validation-only L0 diagnosis and
  next-step decision.
- `tables/verification.json` is the post-run structural audit for steps, episode
  counts, paired day counts, native hashes and finite-check failures.
